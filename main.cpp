...
void task_modbusrtu_routine()
{
    ...
    for(;;)
    {
        rd_result = modbusrtu_m_s2.readHoldingRegisters(0x0000, 2);
        if (rd_result == modbusrtu_m_s2.ku8MBSuccess)
        {
            modbustcp_s.Hreg(0, modbusrtu_m_s2.getResponseBuffer(0));
            modbustcp_s.Hreg(1, modbusrtu_m_s2.getResponseBuffer(1));
        }
        else
        {
            //nothing
